...
router.get('/dashboard', authorize('student'), (req, res) => {
  ...
  if (!student) {
    const result = db.prepare('INSERT INTO students (user_id, student_code, program, year_of_study, tuition_balance) VALUES (?, ?, ?, ?, ?)')
      .run(req.session.user.id, `STU${req.session.user.id}`, 'General Studies', 1, 0);
    student = db.prepare('SELECT * FROM students WHERE id = ?').get(result.lastInsertRowid);
  }

  const enrollments = db.prepare(`
    SELECT e.id, e.status, c.code, c.name, c.credits, c.prerequisite, g.grade
    FROM enrollments e
